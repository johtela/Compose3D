namespace ComposeFX.Geometry
{
	...
	public delegate V Manipulator<V> (V input) where V : struct, IVertex3D;
	...
	public static class Manipulators
	{
		public static Geometry<V> ManipulateVertices<V> (this Geometry<V> geometry, Manipulator<V> manipulator, 
			bool recalculateNormals) where V : struct, IVertex3D
		{
		...
		{
			return v => v.With (
				v.position.Jitter (range), 
				v.normal);
		}

		public static Manipulator<V> JitterColor<V> (float range)
			where V : struct, IVertex3D, IDiffuseColor<Vec3>
		{
			return v =>
			{ 
				v.diffuse = v.diffuse.Jitter (range);
				return v;
			};
		}
	}
